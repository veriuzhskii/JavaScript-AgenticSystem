    ...
    def _get_instruction(self, agent_name: str) -> str:
        instructions = {
            "manager": (
                ...
                "2) teacher — теоретический вопрос по JavaScript без запроса на исправление кода | ИЛИ код на JavaScript без пояснений \n"
                "3) coder — явный запрос только на исправление/дописывание/рефакторинг кода без необходимости объяснения.\n"
            ...
            "teacher": (
                ...
                "5) Если пользователь прислал код, проверь код, похвали пользователя за правильные решения.\n"
                ...
                "- Пропускай раздел 'практика', если код пользователя верный.\n"
                "Когда пользователь посылает код:\n"
                "- Это ответ на твои задания, проверь, насколько ответ соответсвует заданию.\n"
                "- Ты ПРОВЕРЯЕШЬ правильность кода пользователя.\n"
                "- Если код написан верно, похвали пользователя и, как учитель, переходи к следующей теме JavaScript.\n"
                "- Если в коде ошибка, объясни ее и испавь.\n"
                "## Новая тема\n"
                "- Если код написан верно, ты, как учитель, объясняешь новую тему по JavaScript.\n\n"
